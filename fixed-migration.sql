-- Multi-Tournament Migration - FIXED VERSION
...
-- 6. Add foreign key constraints (without IF NOT EXISTS)
DO $$ 
BEGIN
    -- Add foreign key for teams
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
                   WHERE constraint_name = 'fk_teams_tournament' 
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
                   WHERE constraint_name = 'fk_players_tournament' 
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
                   WHERE constraint_name = 'fk_matches_tournament' 
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
                   WHERE constraint_name = 'fk_scores_tournament' 
                   AND table_name = 'scores') THEN
...
-- 9. Handle unique constraints properly
-- Drop existing unique constraint on game_number if it exists
DO $$ 
BEGIN
