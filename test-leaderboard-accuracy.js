...
const mockMatches = [
  ...
  // Friday PM Foursomes - Team A vs Team B vs Team C (3-way)
  {
    id: 2,
    teamAId: 1,
    teamBId: 2,
...
function testCumulativeTotals() {
  ...
  const teamCTotalPoints = twoWayResult.teamCPoints + threeWayResult.teamCPoints;
  ...
  console.log(`   Team C: ${teamCTotalPoints} points (${twoWayResult.teamCPoints} + ${threeWayResult.teamCPoints})`);
  
  // Expected totals
  const expectedTeamATotal = 5 + (3 * 2); // 5 (4BBB win) + 6 (2 Foursomes wins) = 11
  const expectedTeamBTotal = 0 + 0; // 0 (4BBB loss) + 0 (2 Foursomes losses) = 0
